...
def sector_chosen(argument):
    ...
    df_Makanan = pd.read_csv('Dataset/fix 02 - Makanan.csv')
    df_Minuman = pd.read_csv('Dataset/fix 02 - Minuman.csv')
    df_Rumah = pd.read_csv('Dataset/fix 03 - Rumah.csv')
    df_Sandang = pd.read_csv('Dataset/fix 04 - Sandang.csv')
    df_Transportasi = pd.read_csv('Dataset/fix 07 - Transportasi.csv')
...
def city_comparison(city):
    dataset_1 = pd.read_csv('Dataset/longitude, latitude.csv')
    dataset_2 = pd.read_csv('Dataset/longitude, latitude.csv')
...
def get_sector_value(product):
    ...
    dataset= pd.read_csv('Dataset/Product Names - Sheet1.csv')
...
def marks():
    ...
    if request.method == "POST":
        ...
        if (value =="" and product =="") or city=="" or sector=="" or end_date=="":
            ...
            value = float(value)
            if value == 0:
                return render_template("index.html",my_marks = "Sorry, but There is no Product")
            else:
                end_date = pd.to_datetime(end_date)
                start_date = pd.to_datetime(start_date)
                second_city,third_city = city_comparison(city)
